...
namespace AdvertisementMessage
{
	class Program
	{
		static void Main(string[] args)
		{
			...
			for (int i = 0; i < n; i++)
			{
				int randomIndexPhrase = rand.Next(0, phrases.Length);
				int randomIndexEvent = rand.Next(0, events.Length);
				int randomIndexAuthor = rand.Next(0, authors.Length);
				int randomIndexCity = rand.Next(0, cities.Length);
				String message = $"{phrases[randomIndexPhrase]} {events[randomIndexEvent]} " +
